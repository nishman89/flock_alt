...
function renderFlocks() {
  ...
  items = [...items].sort((a, b) => {
    ...
    // soonest next roost
    const nextA = a.roosts[0]?.date || '9999';
    const nextB = b.roosts[0]?.date || '9999';
    ...
    return `
      <a id="event-card-${f.id}" class="event-card" href="event.html?id=${f.id}">
        ...
        <div class="card-body">
          ...
          <div class="card-foot">
            <span style="font-size:13px;color:var(--text3)">${f.roosts.length} upcoming Roost${f.roosts.length !== 1 ? 's' : ''}</span>
